...
const SectionSegment = ({textPosition = 'left', image, text="text", header="header", link=""}) => {
    ...
    const renderText = (txt, head, link) => {
        return (
            <div className={`${textClasses} width position-absolute mt-0 mt-lg-5 pt-0 pt-md-5 h-100 `}>
                <h2 className="ms-5 ps-5 mb-4 mb-lg-5 mt-5 w-75 fs-1">{head}</h2>
                <p className="ms-5 ps-5 w-75 fs-4 mb-4 mb-lg-4 max-width">
                    {txt}
                    {renderButton(link)}
                </p>
